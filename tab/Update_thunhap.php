<?php
  ...
  include_once "../inc/database.php";
      ...
      $idk = $_SESSION['p'];;
      echo $_GET["$idk"];
      $q = Database::query("SELECT * FROM ktragiuaky.khoan_thunhap where Id_khoan= ".$_GET["$idk"]."");
      $r= $q->fetch_array();
...
<nav class="navbar navbar-light bg-light justify-content-between nav" style="display: block; ">
  ...
  <form action="edit_thunhap.php" method="post">
      ...
      <div class="container" style="width:30%">
                  <div class="form-group">
                      ...
                      <input class="form-control" type="datetime-local" name="txtDate" value="<?php echo $r['ngaySD']?>">
                      <span>Loại tiền sử dụng</span>
                      <select name="mySelect" class="form-select" aria-label="Default select example">
                        <?php
                            if($idk == "mainIncome"){
                              $q = Database::query("SELECT tenloai,Id_loai FROM ktragiuaky.loai_thunhap where type = 0 and User_id = ".$_SESSION['id']."");
                            }else{
                              $q = Database::query("SELECT tenloai,Id_loai FROM ktragiuaky.loai_thunhap where type = 1 and User_id = ".$_SESSION['id']."");
                            }
